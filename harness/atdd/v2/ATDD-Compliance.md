...
> **状态**: Draft
> **对应模块**: M14
> **对应 TDS**: `docs/architecture/TDS/TDS-M14-compliance.md`
...
### Scenario: L-Crisis-Response-A 用户点击 [我需要帮助]（M14-FR-06 新增）
```gherkin
Given 用户看到危机响应卡
When 用户点击 [我需要帮助]
Then 展示完整资源列表
And 不恢复 AI 对话
And 记录 safety_audit_logs.action_taken="professional_help"
```

### Scenario: L-Crisis-Response-B 用户点击 [我只是想聊聊]（M14-FR-06 新增）
```gherkin
Given 用户看到危机响应卡
When 用户点击 [我只是想聊聊]
Then 恢复轻度 AI 陪伴
And 继续监控用户输入
And 记录 safety_audit_logs.action_taken="chat_continue"
```

### Scenario: L-Crisis-Forbidden 禁用回复模式（M14-FR-06 新增）
```gherkin
Given L-Crisis 触发
When AI 生成回复
Then 禁止出现："我无法回答医疗问题，建议您咨询专业医师"
And 禁止出现："建议您拨打心理援助热线"（单一资源，无情感连接）
And 禁止出现：任何 AI 生成的建议、分析、或共情长文
And 禁止出现：任何打卡提醒或方案推送
```

### Scenario: 危机词表独立管理（M14-FR-01 新增）
```gherkin
Given 合规引擎加载敏感词库
Then mental-health-crisis-words.yaml 独立加载（最高优先级）
And medical-reject-words.yaml §dangerous_behavior 分组
And medical-reject-words.yaml §medication_treatment 分组
And 三套词表独立版本号管理
```

### Scenario: 词表季度审查（M14-FR-07 新增）
```gherkin
Given 运营团队季度审查
When 审查危机词表
Then mental-health-crisis-words.yaml 关键词覆盖率自检
And medical-reject-words.yaml 新增词条评审
And 审查记录归档（不含词表原文）
```
...
## 八、forbidden-words 与 ack-pool 引用
...
### 相关定义
- 合规红线：详见 [ATDD-Shared.md §三](../ATDD-Shared.md#三合规红线唯一真源m5m6m7m8m10m14共享)
- ACK禁用词：详见 [ATDD-Shared.md §三.2](../ATDD-Shared.md#三合规红线)
- 错误码定义：详见 [ATDD-Shared.md §四](../ATDD-Shared.md#四错误码字典)
- 危机词表管理：详见 [TDS-M14-compliance.md §4.2](../architecture/TDS/TDS-M14-compliance.md#42-危机分级)
- 语音合规：详见 [ATDD-Conversation.md §语音合规](../ATDD-Conversation.md#x-语音合规引用-m14)

### 技术架构说明

> ⚠️ **TDS §7 与本文档 Scenario 重复，已废弃。** 业务层 Scenario 以本文档 §一~§六 + §八为唯一真源。技术层 TODO 见 TDS-M14-compliance.md §2~§5。
...
| 2026-07-21 | V1.1 | P0修复：补全C-5禁止词\"漂亮\"；新增L-Crisis两步响应+禁用回复模式+词表管理+季度审查；修正章节编号+引用规范 |
