...
def check_input(text: str) -> dict[str, bool | Severity]:
    """
    检查用户输入是否违规。
    ...
    """
    critical_keywords = [
        "治疗", "治愈", "治病", "处方", "医生", "医院", "开药", "开点药",
        "微整", "整形", "打针", "玻尿酸", "瘦脸针", "超声刀", "热玛吉",
        "几分", "颜值",
        "几天见效", "几天能见效", "一定", "一定变白", "保证", "保证有效", "永久", "复发", "根治",
        "瘦下来", "脸变小",
    ]
    matched = [kw for kw in critical_keywords if kw in text]
    if matched:
        return {"blocked": True, "severity": "critical", "matches": matched}
...
def check_output(text: str) -> dict[str, bool | Severity | list[str]]:
    ...
    """
    critical_keywords = [
        "治疗", "治愈", "治病", "处方", "微整", "整形", "打针", "玻尿酸",
        "瘦脸针", "永久", "复发", "根治", "保证", "治好", "变白",
        "瘦下来", "脸变小",
    ]
    warning_keywords = [
        "几天见效", "一定变白", "保证有效", "最好", "唯一",
        "见效", "好一些", "同类最好",
    ]
