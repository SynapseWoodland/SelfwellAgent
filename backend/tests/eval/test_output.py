...
from app.services.compliance.checker import check_output
...
SCRIPT_CASES = [
    ...
    pytest.param(
        "G-S-03",
        "想休息一天也可以，先把今天的你抱抱。我们慢慢来。",
        "pass",
        id="G-S-03 合规话术-接纳",
    ),
    pytest.param(
        "G-S-04",
        "今天完成了 1 个动作，已坚持 3 天，真的很棒。",
        "pass",
        id="G-S-04 合规话术-坚持",
    ),
    pytest.param(
...
def test_llm_output_compliance(case_id: str, text: str, expected: str) -> None:
    ...
    if expected == "block":
        assert result["blocked"], f"[{case_id}] 输出未被拦截（期望 block）：{text}"
        assert result.get("severity") == "critical", (
            f"[{case_id}] 严重违规输出仅被 warn 而非 block：{text}"
        )
    else:
        assert not result["blocked"], f"[{case_id}] 合规输出被错误拦截（期望 pass）：{text}"
...
def test_no_regression_on_known_clean() -> None:
    ...
    for entry in ALL_CASES:
        ...
        if expected != "pass":
            continue
        result = check_output(text)
        assert not result["blocked"], f"回归失败（误杀）[{case_id}]：{text}"
...
def test_no_regression_on_known_bad() -> None:
    ...
    for entry in ALL_CASES:
        ...
        if expected != "block":
            continue
        result = check_output(text)
        assert result["blocked"], f"回归失败（漏过）[{case_id}]：{text}"
        assert result.get("severity") == "critical", f"回归失败（级别错误）[{case_id}]：{text}"
