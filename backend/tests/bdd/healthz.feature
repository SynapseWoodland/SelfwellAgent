Feature: Healthz Probe — 四段探针决策树

  Scenario: 所有依赖服务（db/redis/minio/llm）均可达
    Given 后端服务已启动
    And PostgreSQL 可达
    And Redis 可达
    And MinIO 可达
  ...
  Scenario: 仅 MinIO 降级时返回 200（degraded）—— storage 可降级
    Given 后端服务已启动
    And PostgreSQL 可达
    And Redis 可达
    And MinIO 不可达
