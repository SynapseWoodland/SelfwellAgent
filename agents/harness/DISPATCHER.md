...
> **真源**：`docs/harness/workflow-v2.yaml` 的 `phases[].exit_criteria`（16 个 phase 各自 3 条命令）。
...
def is_phase_complete(current_phase: str, workflow: dict) -> bool:
    ...
    Args:
        ...
        workflow: 读自 docs/harness/workflow-v2.yaml 的 phases 列表
...
3. 真正跨平台时用 Python 脚本替代（见 `docs/harness/scripts/check_phase.py` 计划）
