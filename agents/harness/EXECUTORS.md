...
# EXECUTORS — 6 执行角色协议（V3）

> **V3 更新**：新增 `REVIEW_PLAN` phase；证据路径迁移到 `templates/`。
> 6 个 executor 对应 `PLAN` / `CODE` / `VERIFY` / `DEPLOY` / `SECURITY_TEST` / `REGRESSION`；**只调用既有 Skill**，不重复造轮子；**所有业务代码 / 业务规则落 `backend/app/`、`apps/`、`rules/`**（R-2 红线）。

## 一、6 执行角色总览

| # | 角色 ID | phase | 主用 Skill | evidence |
|---|---------|-------|-----------|----------|
| 1 | `plan-generator` | `PLAN` / `REVIEW_PLAN` | `ad-tdd` Phase 1 | `templates/phase/05-plan.md` + `templates/review/review-plan.md` |
| 2 | `developer` | `CODE` | `ad-tdd` Phase 3 + `coding-standards` | 业务代码 + `templates/phase/06-code.md` |
| 3 | `deployer` | `DEPLOY` | `pr-gate` + Docker / Alembic | `templates/phase/09-deploy.md` |
| 4 | `verifier` | `VERIFY` | `coding-standards` GATES L0-L6 + `pr-gate` | `templates/phase/07-verify.md` |
| 5 | `security-reviewer` | `SECURITY_TEST` | `python-patterns` 安全规则 | `templates/phase/08-security-test.md` |
| 6 | `tester` | `REGRESSION` | `golden-set` + Playwright | `templates/phase/10-regression.md` |
...
ATDD 阶段**不跑自动化测试**，只产出 `.feature` 验收标准文件 + `templates/phase/04-atdd.md` evidence。
...
### 2.1 plan-generator（PLAN / REVIEW_PLAN）

- **Read**：`templates/phase/04-tds.md`（TDS）+ 上游 evidence
- **Write**：`templates/phase/05-plan.md` + `templates/review/review-plan.md`（REVIEW_PLAN）
...
- **Write**：`backend/app/`、`apps/`、测试代码、`templates/phase/06-code.md`
- **禁用**：Read `templates/phase/*.md` 原文（避免上下文污染）；任何 R-1~R-5 违规
- **增量识别**：基于 git diff 确定增量测试文件（`git diff --name-only HEAD~1...HEAD | grep "^backend/tests/unit"`），无改动则全量
- **Skill 调用**：
...
- **增量识别**：基于 git diff 确定增量测试文件（`git diff --name-only HEAD~1...HEAD | grep "^backend/tests/"`）
- **增量 pytest**：只运行改动的测试文件，无改动则跳过
- **Run**：`uv run pytest tests/{integration,e2e,smoke} -x -q`（仅运行增量测试）；`python -m eval.runner --mode pr`（仅 Prompt 改动时）
- **Read**：`templates/phase/04-tds.md`（TDS）、`templates/phase/06-code.md`（developer 自审报告）
- **Write**：`templates/phase/07-verify.md`
...
| L5 | grep 12 条架构&安全（详见 `l0-l6-gates.mdc` §L5） | FAIL → 退回 developer |
| L6 | `uv run pytest --cov=app --cov-report=term-missing --cov-fail-under=60`（硬卡数字） | FAIL → 补测试或降门（走 ADR） |

- **退出**：✅ L0-L6 全 PASS + ATDD 全过 + PR-Gate 7 项 ✅；🔴 任一 L 级 FAIL → 退回 developer

### 2.4 security-reviewer（SECURITY_TEST）

> **并行执行**：与 VERIFY 并行，两者都完成后进入 REGRESSION。

- **Run**：`uv run ruff check . --select=S,SEC,B`（安全扫描）
- **Read**：`templates/phase/06-code.md`（developer 自审报告）
- **Write**：`templates/phase/08-security-test.md`
- **禁用**：写 feature 代码
...
**产出**：`templates/phase/09-deploy.md`（包含"跳过"或"执行"记录）
...
- **Read**：`templates/phase/06-code.md`（如涉及代码文档）
- **Write**：`templates/phase/12-doc-fix.md`
...
- **Write**：`templates/phase/10-regression.md`
- **禁用**：写 feature 代码
...
|-------|----------|--------------|
| `PLAN` | plan-generator | `templates/phase/05-plan.md` |
| `REVIEW_PLAN` | plan-generator | `templates/review/review-plan.md` |
| `CODE` | developer | `templates/phase/06-code.md` |
| `DEPLOY` | deployer | `templates/phase/09-deploy.md` |
| `VERIFY` | verifier | `templates/phase/07-verify.md` |
| `SECURITY_TEST` | security-reviewer | `templates/phase/08-security-test.md` |
| `DOC_FIX` | developer | `templates/phase/12-doc-fix.md` |
| `REGRESSION` | tester | `templates/phase/10-regression.md` |
...
| Executor | 终止行为 |
|----------|---------|
| `plan-generator` | 写 `templates/phase/05-plan.md` + 追加 TDS 实施计划 → 进入 REVIEW_PLAN |
| `plan-generator`（REVIEW_PLAN） | 写 `templates/review/review-plan.md` → 进入 PRE_MORTEM |
| `developer`（CODE） | 写 `templates/phase/06-code.md` + L0-L4 PASS + 覆盖率达标 → 进入 DEPLOY |
| `deployer` | 写 `templates/phase/09-deploy.md` + 部署成功/跳过 → 进入 VERIFY |
| `verifier` | 写 `templates/phase/07-verify.md` + L0-L6 + PR-Gate ✅ → 进入 REGRESSION |
| `security-reviewer` | 写 `templates/phase/08-security-test.md` + 安全扫描 PASS → 进入 REGRESSION |
| `developer`（DOC_FIX） | 写 `templates/phase/12-doc-fix.md` + 文档检查 PASS → 进入 SIGN_OFF |
| `tester` | 写 `templates/phase/10-regression.md` + baseline 跌幅 ≤ 5% → 进入 SIGN_OFF |

## 六、与其他协议文件边界

```
DISPATCHER → 状态机路由（"现在轮到哪个 executor"）
EXECUTORS（本文件）→ 6 执行角色（"我做什么 / 不做什么"）
REVIEWERS → 6 评审角色（"评审只产出观点，不写代码"）
ORCHESTRATOR → 跨角色合成（PRE_MORTEM / SIGN_OFF）
```

四份协议通过 `templates/phase/*.md` 路径对齐，单文件改动不强制其他四份同步。

## 七、参考

- 评审角色：`agents/harness/REVIEWERS.md`
- 合成协议：`agents/harness/ORCHESTRATOR.md`
- 调度协议：`agents/harness/DISPATCHER.md`
- Skill 入口：`ad-tdd` / `coding-standards` / `pr-gate` / `golden-set` / `frontend-standards`
- 状态机：`harness/workflow-v3.yaml`
