...
| `docs/adr/0007-vision-pipeline-split.md` | 已 sync 字段名（PR3） | — |
| `docs/api/sse-events.md §5` | assistant 域 SSE 契约（PR3 已追加）| **PR4 不改**（schema 没变；新字段 `is_fallback`/`fallback_reason` 在 end event payload 内，§5 已写 end 字段"可选扩展"） |
...
| Doc | `docs/api/sse-events.md §5` 增 §5.5「fallback 协议」（可选小节）：end event payload 可选字段 `is_fallback` + `fallback_reason` 说明 | `docs/api/sse-events.md` | +~30 行 |
...
- docs/api/sse-events.md §5.5 追加 fallback 协议说明
...
- [ ] `docs/api/sse-events.md §5.5` 已追加 fallback 协议
