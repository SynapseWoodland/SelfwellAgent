...
**当前已知测试失败**（v4 bug，待 v5 修）：
- `less file.log` — `less` 正则 `\ble\s` 误写为 `le` + 边界，应当用 `(?:^|\s|;|&&|\|\|)less\s` 重写
- `echo hi | tee out.txt` — `tee` 通过 pipe 形式被白名单分支误判放行，需在白名单分支内额外检查 `tee` 命中
- `dir /b C:\Users` — cmd 反斜杠路径在 JSON 字符串中双重转义，正则未覆盖
...
| **v4** | **2026-07-15** | **Python 重写 + stdin/stdout JSON + matcher 精准触发** | **当前使用（有 3 个已知 bug 待 v5 修）** | **a13c31e** |
| v5 (待) | — | 修 `less` / `tee` pipe / `dir /b` 漏检 + 加 `cursor-hook-bypass` 注释前缀 | 规划中 | — |
