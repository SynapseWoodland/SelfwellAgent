...
- 与 Flutter 端（`packages/api-types/dart/`）并行；各自 README 指向 SF1 对齐

## SF1 接入 checklist

- [ ] 后端 S1 落地 `auth.wxMpLogin` / `users.getCurrentUser` / `users.updatePushToken`
- [ ] 在 CI 加 step：`openapi-generator-cli generate -i ../../docs/architecture/api.yaml -g typescript -o ./ts --additional-properties=supportsES6=true`
